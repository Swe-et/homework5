...
class SurnameClassifier(nn.Module):
    ...
    def forward(self, x_in, x_lengths=None, apply_softmax=True):
...
# 输出结果
print(f"损失值: {train_state['test_loss']:.4f}")
print(f"分类准确性: {train_state['test_acc']:.4f}")


def predict_nationality(surname, classifier, vectorizer, device):
    # Vectorize the surname
    vectorized_surname, vec_length = vectorizer.vectorize(surname)

    # Move the tensors to the appropriate device (either cpu or cuda)
    ...
    logits = classifier(vectorized_surname, vec_length, apply_softmax=False)

    # Apply softmax to convert logits to probabilities
    probabilities = F.softmax(logits, dim=1)

    # Get the probability values and indices of the predicted nationality
    probability_values, indices = probabilities.max(dim=1)

    # Extract the predicted nationality and probability
    index = indices.item()
    prob_value = probability_values.item()

    # Lookup the nationality from the vectorizer
...
for surname in surnames:
    result = predict_nationality(surname, classifier, vectorizer, device)
    print(f"姓氏: {result['surname']}, 国籍: {result['nationality']}, 置信度: {result['probability']:.4f}")
